...
var PNode = (_PNode = {}, _defineProperty(_PNode, Symbol.iterator, function () {
  ...
  return this.isRadical() || this.isPGCD() || this.isMin() || this.isMax() || this.isMod() || this.isCos() || this.isSin() || this.isTan() || this.isLog() || this.isLn() || this.isExp() || this.isFloor() || this.isAbs();
...
}), _defineProperty(_PNode, "shuffleFactors", function (_shuffleFactors) {
  function shuffleFactors() {
    return _shuffleFactors.apply(this, arguments);
  }

  shuffleFactors.toString = function () {
    return _shuffleFactors.toString();
  };

  return shuffleFactors;
}(function () {
  return shuffleFactors(this);
})), _defineProperty(_PNode, "shuffleTermsAndFactors", function (_shuffleTermsAndFactors) {
  function shuffleTermsAndFactors() {
    return _shuffleTermsAndFactors.apply(this, arguments);
  }

  shuffleTermsAndFactors.toString = function () {
    return _shuffleTermsAndFactors.toString();
  };

  return shuffleTermsAndFactors;
}(function () {
  ...
  if (this.isNumber()) {
    ...
    var _regexs = [/\d{4}/, /\s$/, /\s\d{2}$/, /\s\d{2}\s/, /\s\d$/, /\s\d\s/];
    if (_regexs.some(function (regex) {
      return _int.match(regex);
    })) return true;

    if (dec) {
      _regexs = [/\d{4}/, /^\s/, /^\d{2}\s/, /\s\d{2}\s/, /^\d\s/, /\s\d\s/];
      if (_regexs.some(function (regex) {
...
function createNode(params) {
  ...
  if (node.exclude) {
    var _iterator2 = _createForOfIteratorHelper(node.exclude),
        _step2;
  ...
  if (node.excludeCommonDividersWith) {
    var _iterator3 = _createForOfIteratorHelper(node.excludeCommonDividersWith),
        _step3;
