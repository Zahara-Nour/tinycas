...
var COMP = (0, _lexer.token)('@[<>]=?'); // const ANTISLASH = token('\\')
...
var CLOSING_CURLYBRACKET = (0, _lexer.token)('}'); // const INTEGER_TEMPLATE = '@\\$(e[pi])(r)?'
...
var FUNCTION = (0, _lexer.token)('@cos|sin|sqrt|pgcd|mini|maxi|cos|sin|tan|exp|ln|log|mod|floor|abs'); // NUMBER      = token("\\d+(\\.\\d+)?"); // obligé de doubler les \ sinon ils sont enlevés de la chaine
// const DECIMAL = token('@[\\d]+[,\\.][\\d]+') // obligé de doubler les \ sinon ils sont enlevés de la chaine

var INTEGER = (0, _lexer.token)('@[\\d]+'); // obligé de doubler les \ sinon ils sont enlevés de la chaine
...
function parser() {
  ...
  function parseRelation() {
    ...
    switch (relation) {
      case "!=":
        e = (0, _node.unequality)([e, parseMember()]);
        break;

      case '=':
        e = (0, _node.equality)([e, parseMember()]);
        break;
      ...
      case '<=':
      case '>=':
        e = (0, _node.inequality)([e, parseMember()], relation);
    }
