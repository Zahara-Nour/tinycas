(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.tinycas = factory(global.Decimal));
  ...
  function evaluate(node, params) {
    switch (node.type) {
      ...
      case TYPE_COS: {
        return evaluate(node.first).cos()
      }

      case TYPE_SIN: {
        return evaluate(node.first).sin()
      }

      case TYPE_TAN: {
        return evaluate(node.first).tan()
      }

      case TYPE_LN: {
        return evaluate(node.first).ln()
      }

      case TYPE_EXP: {
        return evaluate(node.first).exp()
      }
  ...
  */
      ...
      case TYPE_MOD: {
        ...
        if (node.isNumeric()) {
          if (
            ...
            a = new Decimal__default["default"](a.string);
            b = new Decimal__default["default"](b.string);
  ...
  function compose(node, g, variable = 'x') {
  ...
  function derivate(node, variable = 'x') {
    let e;
  ...
  const TYPE_GCD = 'gcd';
  const TYPE_MAX = 'maxi';
  const TYPE_MAXP = 'maxip';
  const TYPE_MIN = 'mini';
  const TYPE_MINP = 'minip';
  const TYPE_MOD = 'mod';
